describe('Blog app', function() {
  beforeEach(function() {
    ...
    const user = { username: 'enghup',
      name: 'Hello',
      password:'ferries' }

    const invUser =  { username: 'John',
    name: 'Price',
    password:'ferries' }
    ...
    Cypress.Commands.add('login',({ username,name,password,url }) => {
      cy.request('POST','http://localhost:3000/api/login',{ username,name,password })
        .then(({ body }) => {
  ...
  describe('logged in', function(){
    beforeEach(function(){
      ...
      cy.addBlog({ author:'JK Rowling',title: 'Hairy Pottery',url:'meh',like:0 })
    })
    ...
    describe('Checking sort function',function(){
      beforeEach(function(){
        cy.addBlog({ author:'JENM',title: 'WEEEEEEEEE',url:'meh',like:0 })
        cy.addLike({ author:'JK Rowling',title: 'Hairy Pottery',url:'meh',count:3 })
      })
      it.only('Sorted blogs', function(){
        cy.get('.blogs')
        .children(0)
        .should('contain', 'Hairy Pottery')
